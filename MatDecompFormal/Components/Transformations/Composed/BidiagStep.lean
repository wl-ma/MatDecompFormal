...
本文件定义了 `BidiagStepTransform`，这是一个在 `Fin n` 世界中实现的宏观变换，
代表了双对角化算法的一个完整归纳步骤。
-/
...
-/
noncomputable def BidiagStepTransform (n m : ℕ) (R : Type*) [NeZero n] [NeZero m]
    [Field R] [DecidableEq R]
    (h_col_pivot_nz : ∀ (A : Matrix (Fin n) (Fin m) R), (∃ i, i ≠ 0 ∧ A i 0 ≠ 0) → A 0 0 ≠ 0)
    (h_row_pivot_nz : ∀ (A : Matrix (Fin n) (Fin m) R), (∃ j, j ≠ 0 ∧ A 0 j ≠ 0) → A 0 0 ≠ 0) :
    Transformation (Matrix (Fin n) (Fin m) R) :=
  Transformation.compose_sequential
    (AnnihilateColumnTransform n m R h_col_pivot_nz)
    (AnnihilateRowTransform n m R h_row_pivot_nz)
