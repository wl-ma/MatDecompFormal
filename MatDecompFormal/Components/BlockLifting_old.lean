...
variable (e_ι : ι ≃o ({i // p_ι i} ⊕ {i // ¬p_ι i}))
variable (e_κ : κ ≃o ({j // p_κ j} ⊕ {j // ¬p_κ j}))
...
def lift_block
    ...
    (M₂₁ : Matrix {i // ¬p_ι i} {j // p_κ j} R) (M₂₂ : Matrix {i // ¬p_ι i} {j // ¬p_κ j} R) :
    Matrix ι κ R :=
  (fromBlocks M₁₁ M₁₂ M₂₁ M₂₂).reindex e_ι.symm.toEquiv e_κ.symm.toEquiv
...
theorem lift_block_isUpperTriangular_iff [FinEnum ι] [DecidablePred p_ι]
  ...
  · -- (→) 假设提升后的矩阵是上三角的
    ...
    · -- 证明 M₁₁ 是上三角的
      ...
      have h_order : e_ι.symm (Sum.inl j) < e_ι.symm (Sum.inl i) := by sorry
    ...
    · -- 证明 M₂₂ 是上三角的
      ...
      have h_order : e_ι.symm (Sum.inr j) < e_ι.symm (Sum.inr i) := by sorry
        -- rw [e_ι.symm.lt_iff_lt]; exact hij
      specialize h h_order
      simpa [reindex_apply, Equiv.symm_apply_eq, fromBlocks_apply₂₂] using h
    · -- 证明 M₂₁ = 0
      ...
      have h_order : e_ι.symm (Sum.inl j) < e_ι.symm (Sum.inr i) := by sorry
...
lemma lift_block_preserves_IsUnitLowerTriangular [FinEnum ι] [DecidablePred p_ι]
  ...
  · funext i
    ...
    -- 对 i 的来源进行分情况讨论，并为每个分支的等式命名
    rcases h_eq : e_ι i with (i₁ | i₂)
    · -- Case 1: i 来自左上块 (p_ι i 为真)
      -- 目标: fromBlocks ... (e_ι i) (e_ι i) = 1
      -- h_eq : e_ι i = Sum.inl i₁
      -- 使用 simp 战术，并明确告诉它使用 h_eq 和 fromBlocks_apply₁₁
      simp [h_eq, fromBlocks_apply₁₁, one_apply_eq]
    · -- Case 2: i 来自右下块 (¬p_ι i 为真)
      -- 目标: fromBlocks ... (e_ι i) (e_ι i) = 1
      -- h_eq : e_ι i = Sum.inr i₂
      -- 同样，使用 simp 并提供所有必要的重写规则
      rw [submatrix_apply, h_eq, fromBlocks_apply₂₂, ← diag_apply L' i₂, hL'.2]
      simp
...
lemma lift_diag_preserves_IsUnitLowerTriangular [FinEnum ι] [DecidablePred p_ι]
  ...
  · funext i
    ...
    · -- Case 1: i 来自左上块 (p_ι i 为真)
      ...
      -- h_eq : e_ι i = Sum.inl i₁
      -- 使用 simp 战术，并明确告诉它使用 h_eq 和 fromBlocks_apply₁₁
      rw [submatrix_apply, h_eq, fromBlocks_apply₁₁, ← diag_apply L₁₁, hL₁₁.2]
      simp
    · -- Case 2: i 来自右下块 (¬p_ι i 为真)
      -- 目标: fromBlocks ... (e_ι i) (e_ι i) = 1
      -- h_eq : e_ι i = Sum.inr i₂
